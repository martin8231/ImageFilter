...
public class ImageFilter{
	...
	public void clean(){
       ...
       BufferedImage inImage = Image.readImage("fifa16corrupt.jpg");
       BufferedImage grayImage = Image.toGray(inImage);
       byte[][] input = Image.getGrayByteImageArray2DFromBufferedImage(grayImage);
        ...
        for (int i = 1; i < input.length - 1; i++) {
            for (int j = 1; j < input[0].length - 1; j++) {
                ...
                for (int x = -1; x <= 1; x++) {
                    for (int y = -1; y <= 1; y++) {
                        values.add((input[i + x][ j + y] & 0xff));
                    }
                }
                
                Collections.sort(values);
                
                int avg = getTrimMean(values);
       ...
       BufferedImage outImage6= Image.setGrayByteImageArray2DToBufferedImage(input);
       Image.writeImage(outImage6, "jpg", "fifa16cleaned.jpg"); 
    ...
    public int getTrimMean(ArrayList<Integer> v){
        int sum = 0;
